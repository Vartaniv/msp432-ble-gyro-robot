...
void BLE_UART_Init()
{
        ...
        // Disable the following interrupts by clearing the corresponding bits in the IE register:
        // - Transmit Complete Interrupt (UCTXCPTIE, Bit 3)
        // - Start Bit Interrupt (UCSTTIE, Bit 2)
        EUSCI_A3->IE &= ~0x0C;

        // Enable the following interrupts by setting the corresponding bits in the IE register:
